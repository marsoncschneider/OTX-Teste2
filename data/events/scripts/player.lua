...
function Player:onGainExperience(source, exp, rawExp)
	if not source or source:isPlayer() then
		...
		-- CAST Bonus Experience
        -- if player:startLiveCast(param) then
           -- exp = exp * 2
        -- end
		
		-- local cast = self:getLiveCast()
	    -- if cast then
		   -- exp = exp * 2
	    -- end
		...
		local CastStorage = 17754
		local multiplier = 1.2 -- 10%+ exp
        if player:getStorageValue(CastStorage) == 1 then
           exp = exp * multiplier
        end
	...
	if configManager.getBoolean(configKeys.STAMINA_SYSTEM) then
		...
		elseif staminaMinutes > 1000 then
