...
function onLogin(player)
	...
	local vipMounts = {
		203, 191, 193, 33, 144, 38, 631, 76, 128, 131
	}
	...
	local vipOutfits = {
		1372, 1371, 1490, 1489, 1461, 1460, 1056, 1057, 1210, 1211, 1676, 1675, 1323, 1322, 151, 155, 1456, 1457, 1161, 1162
	}
	...
	if player:getLastLoginSaved() <= 0 then
		...
		if player:getSex() == 1 then
			player:setOutfit({lookType = 974, lookHead = 78, lookBody = 106, lookLegs = 58, lookFeet = 76})
		else
			player:setOutfit({lookType = 975, lookHead = 78, lookBody = 106, lookLegs = 58, lookFeet = 76})
		end
	...
	player:sendTextMessage(messageType or MESSAGE_STATUS_CONSOLE_ORANGE, '[WEBSITE] https://otg-parrot.com/')
	...
	if player:getAccountStorageValue(2) <= 0 then
		...
		player:sendTextMessage(MESSAGE_INFO_DESCR, string.format("You received 7 VIP days."))
	end
