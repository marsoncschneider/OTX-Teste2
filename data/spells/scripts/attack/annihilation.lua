...
function onGetFormulaValues(player, skill, attack, factor, tool)
	...
	local min, max = player:getSpellDamage(-(((skillTotal * 0.16) + 16) + (levelTotal)), -(((skillTotal * 0.20) + 23) + (levelTotal)))

	local skillSecond = skill * element
	local minb, maxb = 0, 0
	if skillSecond > 0 then
		minb, maxb = player:getSpellDamage(-(((skillSecond * 0.16) + 16) + (levelTotal)), -(((skillSecond * 0.20) + 23) + (levelTotal)))
	end
